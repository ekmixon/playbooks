...
def on_start(container):
    ...
    for pan in PAN:
        phantom.debug(f'Firewall {pan} called')
        try:
            response = requests.get(f'https://{pan}{URL}{API}', verify=False, timeout=3)
        ...
        if data:            
            if type(data) is dict:
                phantom.debug(f"Confidence: {data['confidence']} {CONFIDENCE}")
                if int(data['confidence']) > int(CONFIDENCE):
                    for net in botnet_allowlist:
                        phantom.debug(f"{data['src']} {net}")
...
Reboot affected host(s) as needed to fully clear quarantines malware
    ...
    parameters = [
        {
            'short_description': "Issue: Possible botnet activity ("
            + str(results.count('\n') - 1)
            + ")",
...
def send_message_1(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    parameters = [
        {
            'destination': "phantom",
            'message': f"botnet detected: {results_item_1[0]}",
